...
func update_available_aura_breweries():
	...
	auraLabel.text = "Aura Brewery : % (%)\nCreates % aura every % seconds\nAura is rizz per second".format([Game.format_number(GameInstance.data.auraBreweries), Game.format_number(count), Game.format_number(GameInstance.data.auraBreweries* GameInstance.data.multiplier * GameInstance.data.tempMulti), str(5*.98**(GameInstance.data.upgrades[1]*GameInstance.data.goldChains)).left(3)], "%")
...
func _on_breweries_timeout() -> void:
	breweries.start(5*.98**(GameInstance.data.upgrades[1]*GameInstance.data.goldChains))
