...
func mog() -> void:
	if floor(GameInstance.data.moggers * GameInstance.data.aura) > 0:
		Handler.ref.create_rizz(floor(floor(0.1 * (1+GameInstance.data.upgrades[1]) * GameInstance.data.moggers * GameInstance.data.aura * pow(2, GameInstance.data.upgrades[7] * log(GameInstance.data.aura + 1) / log(200)))))
...
func update_available_mogger() -> void:
	...
	moggersLabel.text = "Moggers: %s (%s)\n%s rizz/s" % [
		...
		Game.format_number(floor(0.1 * (1+GameInstance.data.upgrades[1]) * GameInstance.data.moggers * GameInstance.data.aura * pow(2, GameInstance.data.upgrades[7] * log(GameInstance.data.aura + 1) / log(200))))
	]
